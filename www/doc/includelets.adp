...
and a link to delete the element are displayed.
<p>Currently, you can only change element attributes like its title, theme and state from
the page configuration UI.  The only action you can take from the <i>Manage Includelets</i>
page is to delete the element (which perversely can't be done from the page configuration
...
the pageset being configured.  You can add an includelet as a new layout element as many
